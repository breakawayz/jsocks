...
public class SOCKS {
    ...
    static Properties loadProperties(String file_name) {
        ...
        try {
            String jarPath = SOCKS.class.getProtectionDomain().getCodeSource().getLocation().getPath();
            File file = new File(jarPath);
            jarPath = file.getParent();
//          System.out.println("d当前路径"+SOCKS.class.getProtectionDomain().getCodeSource().getLocation().getPath());
//          System.out.println(SOCKS.class.getClassLoader().getResource(jarPath+"/"+file_name).getFile());
            //         InputStream fin = SOCKS.class.getClassLoader().getResourceAsStream(file_name);
            System.out.println(jarPath + "/" + file_name);
            InputStream fin = new FileInputStream(jarPath + "/" + file_name);
            pr.load(fin);
            fin.close();
        } catch (Exception ioe) {
            ioe.printStackTrace();
